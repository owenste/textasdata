...
1. 发展中国家的增长与贸易伙伴的增长显著同步：包含区域共同成分的上限约 0.6；同时排除区域共同冲击与转型初期后，仍有约 0.2–0.3 的联动（登记检验：0.32，95% 区间 0.11–0.53）。
2. 南方伙伴的增长同样带动本国增长，弹性约 0.60；南南联动是多源的，不依赖中国或任何单一南方大国。
3. 随着南南经贸关系的扩展，南方已成为联动的主要来源：按南北弹性相同的口径，南方在伙伴加权增长中的占比由 1990 年代的 45% 升至 21 世纪以来的 77%。
4. 联动以实际经济关系为载体，区域性较强；贸易协定规则深度的独立作用有限，并逐步递减。

其中最稳健的是第 3 项：在南北弹性相同的口径下，南方占比只取决于贸易份额和伙伴增长，与联动弹性的水平无关。
...
由此推出：
- 只要南方在贸易关系中的份额上升、增长更快，无论南北单位联动强度是否相同，**联动来源都会转向南方**；
- 结构转型不以「南北平等」为前提。
...
- **第十四轮补充**：区域×年份 FE 下的结果对样本敏感（探索性诊断，表 44）。
  - 剔除 1991–1996 年转型经济体后，θ 降为 0.14，θ_S 降为 0.11，都不显著；
  - 只用 1997–2023 年时，θ 为 0.24（p = 0.044），θ_S 为 0.19（p = 0.091）。
  - 国家 + 年份 FE 下，结果在所有子样本中都稳健（θ 为 0.47–0.63，p < 0.001）。
  - 所以更准确的表述是：联动在原作者设定下稳健；在更严格的区域×年份 FE 下，全样本和 1997 年以后成立，但部分依赖转型初期的区域同步冲击。
- **南北单位强度差异**（θ_N 1.04 对 θ_S 0.60）：控制大宗商品周期和全球金融周期后，登记检验 E1 为 −0.40（p = 0.064），未达显著。按事先约定，不作为发现，只在讨论中提及（`docs/results_vs_prereg14.md`）。

- **第十五轮：保守量级**（登记检验 C1，`docs/results_vs_prereg15.md`）：只用区域外伙伴、1997–2023 年、区域×年份 FE，θ_OUT = 0.32（p = 0.003，95% 区间 0.11–0.53）；同一设定下，区域外南方伙伴为 0.29（p = 0.003）。
  - 三种各自排除共同冲击的设计（区域外伙伴 0.20、1997 年以后区域×年份 FE 0.24、滞后一期 0.20）量级接近。
  - 结论：**0.6 作为包含区域共同成分的上限，0.2–0.3 作为保守估计，两者并列报告**。
...
| 时期 | 北方伙伴加权增长（百分点） | 南方伙伴加权增长（百分点） | **南方占比（主口径：弹性相同）** | 南方占比（敏感性：估计弹性） |
|---|---|---|---|---|
| 1991–2000 | 1.49 | 1.25 | **45%** | 32% |
| 2001–2010 | 0.73 | 2.39 | **77%** | 65% |
| 2011–2023 | 0.62 | 2.13 | **77%** | 66% |

- **主口径**：南北弹性相同时，南方在联动贡献中的占比就是它在伙伴加权增长中的占比，只要求 θ > 0，与 θ 的水平无关。这与「南北系数大小不作比较」保持一致。
- **敏感性口径**：用估计的南北弹性（0.60、1.04）加权，南方占比为 32% → 66%，结论方向相同。
...
1. **联动存在**：各国现代化不是彼此独立的单元过程。发展中国家的增长与伙伴增长显著同步，扣除区域共同冲击后仍有约 0.2–0.3 的联动；文章描述相互依存的状态，不主张传导的因果强度。
2. **南南联动存在**：带动并非只来自中心国家。南北单位强度的比较证据不足，不下结论。
3. **结构转型**：南方已成为联动的主要来源（主口径 45% → 77%；敏感性口径 32% → 66%）。变化来自关系结构（份额与增长），而不是联动强度。**南南联动是多源的**：拆出中国或各国最大的南方伙伴后，其余南方伙伴的联动依然稳健。
4. **联动的条件**：以实际经济关系为载体，区域性强；制度规则需要落到实际关系上才能发挥作用。

**理论含义**：世界现代化的联动结构正在由「中心带动」向「多元联动」转型。这为「关系性认知」提供了经验支撑，同时补充了它的条件。

## 六、局限与下一步

**局限**：
1. 同期联动不能完全排除共同冲击；年份 FE、区域×年份 FE 和置换检验只能缓解。区域×年份 FE 下的结果对 1991–1996 年转型经济体样本敏感；控制大宗商品和全球金融周期后，核心结果在年份 FE 下不变；
