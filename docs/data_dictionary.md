...
## 23. 新地缘经济变量（第七轮，脚本 41–42）

**原始数据**：`data/raw/geo/imts_trade_1990_2025.csv`（脚本 41 下载）：ISO3（报告方）、partner、year、flow（X = 出口 FOB，M = 进口 CIF）、value_usd（美元，**未**乘以 10^6）。

**`data/clean/geo_new_cy.csv`**：1 行 = 发展中国家 × 年份（1990–2023；理想点只到 2023 年）。
- 伙伴位置 p = (伙伴理想点 − 中国理想点) ÷ (美国理想点 − 中国理想点)；p ≥ 2/3 为美国一侧，p ≤ 1/3 为中国一侧；
- `T_US`、`T_CN`：进出口总额中流向（来自）美国一侧、中国一侧的份额；
- `CONN` 跨阵营平衡度 = 2·min(T_US, T_CN) ÷ (T_US + T_CN)；
- `BRIDGE` 桥接度 = max{√(m_CN·x_US), √(m_US·x_CN)}；`x_US`、`m_CN` 为出口到美国一侧、进口来自中国一侧的份额；
- `GeoV_F` 固定参照脆弱性 = Σ 贸易份额 × |伙伴位置 − 本国 1990–2023 年平均位置|；
- `DIST`：与本国平均位置相距 > 1/3 的伙伴的贸易份额。

效度检验见 `docs/results_vs_prereg7.md`。

## 24. RCEP 月度数据与第九轮变量（脚本 44–46）

- `data/raw/geo/imts_monthly_2017_2026.csv`（不进 git）：IMF IMTS 月度双边贸易。
  - 列：ISO3（报告方）、partner（伙伴）、ym（年-月）、flow（M 为到岸价进口，X 为离岸价出口）、value_usd（美元）；
  - 585 万行，207 个报告方，2017-01 至 2026-05。
- `rcep_data.py` 中构造的变量：
  - **RCEP**：两国都已生效（取较晚的生效月）；
  - **NEW**：中日、日韩；
  - **D21**：2021 年国家对的约束性深度（基础口径）；
  - **CROSS**：一方 p ≤ 1/3、另一方 p > 1/3 的成员对，p 取 2017–2021 年平均；
  - **OTHER**：2017 年以后生效的 DESTA 协定处于生效状态（年度精度）。
- `data/clean/prereg9_family9.csv`：第九族的检验结果与判定。

## 25. `data/clean/linkage_cy.csv` —— 伙伴增长（第十二轮，脚本 50）

- 单位：发展中国家-年度，1991–2023 年；
- 权重：t−3 至 t−1 年对各伙伴的出口份额平均（IMTS），只含有 GMD 增长数据的伙伴；
- 列：
  - gP：伙伴加权增长；
  - gN、gS：北方（23 国）与南方伙伴的部分，gP = gN + gS；
  - gP_noCN：去掉中国一项；
  - sCN：对华出口份额。

## 26. `data/clean/linkage_split_cy.csv` —— 南方伙伴增长的拆分（第十三轮，脚本 53）

- 权重与第十二轮相同，不重新归一；
- 列：
  - gN、gS：北方、南方部分；
  - gCN、gS_noCN：中国部分，以及不含中国的南方部分；
  - gTOP、gS_noTOP：每国每年最大南方伙伴的部分，以及其余南方部分；
  - top、wTOP：最大南方伙伴的代码及其出口份额。
